...
export default function MonthDetailPage() {
  ...
  const { year, month } = useMemo(() => {
    // si no es válido, devolvemos algo inocuo (no debería renderizarse porque redirigimos)
    if (!ym || !isValidYM(ym)) return { year: 0, month: 0 };
    return parseYM(ym);
  }, [ym]);
  ...
  async function load() {
    ...
    try {
      // ✅ segunda barrera: no cargues nada si ym no es válido
      if (!ym || !isValidYM(ym)) {
        setRows([]);
        setMonthRow(null);
        return;
  ...
  // si estamos en redirect, evitamos render feo
  if (!ym || !isValidYM(ym)) {
    return null;
  }
        ...
        {!loading && rows.length > 0 && (
          <div className="border border-black/10 p-4">
            ...
            <ul className="space-y-2">
              {rows.map((r) => {
                const cat =
                  KAKEBO_CATEGORIES[r.category as CategoryKey] ?? null;
